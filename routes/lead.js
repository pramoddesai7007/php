...
router.get('/leadList/foradmin/:companyName', async (req, res) => {
    ...
    try {
      ...
      console.log(leads);
      console.log(`Leads found for company ${companyName}:`, leads);
      res.json(leads);
    } catch (error) {
      console.error('Error in leadList API:', error);
      res.status(500).json({ error: 'Server error' });
    }
  });
...
router.get('/leadList', jwtMiddleware, async (req, res) => {
    try {
      ...
      if (req.user.role === 'admin') {
        ...
        console.log(`Admin Company Name: ${adminCompanyName}, Admin ID: ${empId}`);
        ...
        console.log(`Leads found for admin: ${leads.length}`);
        res.json(leads);
      } else if (req.user.role === 'sub-employee') {
        const userName = req.user.name;
        console.log(`Sub-employee Name: ${userName}`);
  
        // Find leads assigned by the sub-employee
        const leads = await Lead.find({ assignedByName: userName });
        console.log(`Leads found for sub-employee: ${leads}`);
        res.json(leads);
      } else {
...
// GET API to fetch notifications by recipientId
router.post('/notifications/emp', async (req, res) => {
    try {
        const { recipientId } = req.body; // Extract recipientId from the request body

        if (!recipientId) {
            return res.status(400).json({ error: 'recipientId is required' });
        }

        // Query the database to find all notifications with the provided recipientId
        const notifications = await Notification.find({ 'recipientId.recipientId': recipientId });

        if (notifications.length === 0) {
            return res.status(404).json({ message: 'No notifications found for the provided recipientId' });
        }
